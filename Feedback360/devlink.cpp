...
{
    IOHIDDeviceRef tmpRef = IOHIDDeviceCreate(kCFAllocatorDefault, device);
    if (!tmpRef) {
        return false;
    }
    IOReturn ret = IOHIDDeviceOpen(tmpRef, 0);
    if (ret != kIOReturnSuccess) {
        CFRelease(tmpRef);
        return false;
    }
    link->interface = tmpRef;
    return true;
}
...
bool Device_Send(DeviceLink *link,void *data,int length)
{
    if(link->interface==NULL) {
        ...
        IOReturn res = IOHIDDeviceSetReportWithCallback(link->interface, kIOHIDReportTypeOutput, 0, (uint8_t*)data, length, 10000, NULL, NULL);
